...
export default function Navbar({ loading, setLoading, sections, courseUUID, course, fetchCourse }) {
  ...
  useEffect(() => {
    if (sections && sections.length > 0) {
      for (let i = 0; i < sections.length; i += 1) {
        ...
        for (let x = 0; x < section.episodes.length; x += 1) {
          const episode = section.episodes[x];
          if (episode.file === null || episode.content.length === 0) {
            setSend(false);
          }
          if (episode.file || episode.content.length > 0) {
            setSend(true);
          }
        }
      }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    await SetCourseHandle(courseUUID[0], true, 'curriculum');
    await fetchCourse();
    setLoading(false);
    setSend(false);
  };
        ...
        <div className="relative grid  py-2 md:flex md:grid-cols-12 md:gap-8 md:py-0">
              ...
              <div className="w-full ">
                <div className="-ml-4 -mt-2 flex flex-wrap  items-center justify-between sm:flex-nowrap md:hidden">
                  ...
                  <div className="ml-4 mt-2 flex-shrink-0">
                    {send ? (
                      <form className="inline-flex md:hidden" onSubmit={handleSubmit}>
                        {loading ? (
                          <button
                            type="button"
          ...
          <div className="ml-6 hidden md:col-span-2 md:ml-0 md:flex md:items-center md:justify-end">
            {send ? (
              ...
              <div className="text-md mr-4 inline-flex cursor-not-allowed  items-center bg-gray-500 px-6 py-1.5 font-bold text-dark">
                Save
              </div>
            )}
            <Link
