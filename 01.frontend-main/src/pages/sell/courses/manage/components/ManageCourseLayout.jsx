...
import Sidebar from '../../../components/courses/create/sidebar';
import {
  createCourseNFT,
  resetCourseNFT,
  updateCourseKeywords,
  updateCourseSlug,
  updateCourseStatus,
  updateCourseStock,
...
import EditNFTAddress from '@/api/manage/courses/EditNFTAddress';
...
export default function ManageCourseLayout({
  ...
  const courseKeywords = details && details.keywords;
  const courseSlug = details && details.slug;
  ...
  function onChange(e) {
    ...
    if (name === 'slug') {
      ...
      formattedValue = value.toLowerCase().replace(/[^a-z, ]/g, '');
    } else if (name === 'stock') {
  ...
  async function handleSaveChanges() {
    ...
    if (slug !== (details && details.slug) && slug !== '') {
      dispatch(updateCourseSlug(courseUUID[0], formData.slug));
    }

    // Only update the keywords if they have changed and are not empty
    if (keywords !== (details && details.keywords) && keywords !== '') {
      dispatch(updateCourseKeywords(courseUUID[0], formData.keywords));
    }

    // Only update the stock if it has changed
    if (Number(stock) !== Number(details.stock)) {
      dispatch(updateCourseStock(courseUUID[0], formData.stock));
    }

    setLoading(false);
    // history.push(`/sell/courses/manage/settings/${courseUUID}`);
  }

  const publishCourseButton = () => (
    <button
      onClick={async () => {
        if ((details && details.nft_address === '0') || details.nft_address === null) {
          ...
          dispatch(updateCourseStatus(courseUUID[0], true));
        }
      }}
      type="button"
      disabled={!canPublish}
  ...
  const onCreateNFT = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        await EditNFTAddress(courseUUID[0], res.data.results.contractAddress);
  ...
  if (!isAuthenticated && myUser && myUser.role !== 'seller' && myUser.id !== course.details.id) {
    router.push('/sell');
  }
                ...
                <Dialog.Panel className="relative flex w-full max-w-xs flex-1 flex-col dark:bg-dark-bg bg-white">
                  ...
                  <div className="h-0 flex-1 overflow-y-auto">
                    <nav className=" space-y-1 ">
                      <Sidebar course={course} courseUUID={courseUUID} />
                    </nav>
                  </div>
            ...
            <div className="py-8">
              <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="py-4 shadow-button ">
                  ...
                  {course ? <div className="px-8 pt-10">{children}</div> : <div>Loading...</div>}
                          ...
                          <div className="mb-2 w-full">
                            <div className="relative col-span-12 mt-1 rounded-md shadow-sm">
                              ...
                              <input
                                ...
                                max={999999999}
                                placeholder="1000"
                                value={stock || (details && details.stock)}
                                onChange={(e) => {
                                  onChange(e);
                                }}
                                className="ring-none w-full border dark:border-dark-border border-gray-700 py-3 outline-none dark:bg-dark-second pl-2.5 focus:border-gray-500 focus:outline-transparent focus:ring-transparent"
