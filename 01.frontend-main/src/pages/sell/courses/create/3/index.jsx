...
import {
  setCourseStep3,
  setCourseStep3Secondary,
  setCourseStep3Tertiary,
} from '@/redux/actions/create/create';
import FetchPrimaryCategories from '@/api/GetPrimaryCategories';
import FetchSubCategories from '@/api/GetSecondaryCategories';
import FetchThirdCategories from '@/api/GetThirdCategories';
...
export default function CreateCourse3() {
  ...
  const handleSetCategory = () => {
    dispatch(setCourseStep3(setCategory));
    router.push('/sell/courses/create/4');
  };
    ...
    <div className="dark:bg-dark-main">
      ...
      <div className="grid w-full place-items-center py-14">
        <h2 className="font-bold mx-12 text-2xl dark:text-dark-txt md:mx-0 md:text-4xl">
          What category shall this training belong to, young apprentice?
        </h2>
        <p className="font-regular mx-12 mt-4 text-sm md:mx-0 dark:text-dark-txt-secondary md:text-lg">
          Will it be the arts of the force or the piloting of a spaceship?
        </p>
        <p className="font-regular mx-12 mt-2 text-sm md:mx-0 dark:text-dark-txt-secondary md:text-lg">
          The choice is yours, for the power of the dark side is subtle and the category can be
          modified later.
        </p>
        <div className="relative mt-12">
          <select
            ...
            onChange={(e) => {
              onChange(e);
              dispatch(setCourseStep3(e.target.value));
            }}
        ...
        {subcategories && subcategories.length > 0 && (
          <div className="relative mt-6">
            <select
              ...
              onChange={(e) => {
                onChangeSub(e);
                dispatch(setCourseStep3Secondary(e.target.value));
              }}
              className={classNames(
                'focus:border-indigo-500 focus:ring-indigo-500 mt-1 block w-96 rounded-md py-4 px-3 pr-10 text-base focus:outline-none sm:text-sm md:w-[750px]',
                'border-gray-500 dark:border-dark-border',
        ...
        {thirdCategories && thirdCategories.length > 0 && (
          <div className="relative mt-6">
            <select
              ...
              onChange={(e) => {
                onChangeThird(e);
                dispatch(setCourseStep3Tertiary(e.target.value));
              }}
              className={classNames(
                'focus:border-indigo-500 focus:ring-indigo-500 mt-1 block w-96 rounded-md py-4 px-3 pr-10 text-base focus:outline-none sm:text-sm md:w-[750px]',
                'border-gray-500 dark:border-dark-border',
