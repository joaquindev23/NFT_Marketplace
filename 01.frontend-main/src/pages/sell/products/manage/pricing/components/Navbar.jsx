...
export default function Navbar({
  ...
  handleSubmit,
  hasChangesPrice,
  hasChangesComparePrice,
  hasChangesDiscountUntil,
  loading,
}) {
  return (
          ...
          <div className="flex md:static md:inset-y-0 md:left-0 md:col-span-3">
            <div className="flex flex-shrink-0 items-center">
              ...
              >
                ...
                <span className="hidden md:flex">Back to Products</span>
              </Link>
            </div>

            <StatusComponent data={product} />
            <div className="mt-2.5 ml-4 hidden md:flex">
              <span className="truncate text-xs text-dark-txt md:text-sm xl:inline-flex">
                Type of Product: {product && product.details && product.details.business_activity}
              </span>
            </div>
          </div>
                ...
                <div className="-ml-4 -mt-2 flex flex-wrap  items-center justify-between sm:flex-nowrap md:hidden">
                  <div className="ml-4 mt-2">
                    <span className="text-sm text-dark-txt md:hidden md:text-sm xl:inline-flex ">
                      {' '}
                      Type of Product:{' '}
                      {product && product.details && product.details.business_activity}
                    </span>
                  </div>
                  <div className="ml-4 mt-2 flex-shrink-0">
                    {hasChangesPrice || hasChangesComparePrice || hasChangesDiscountUntil ? (
                    ...
                    <Link
                      href={`/sell/products/manage/settings/${productUUID}`}
