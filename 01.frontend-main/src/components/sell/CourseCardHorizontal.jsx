...
export default function CourseCardHorizontal({ data, index }) {
  ...
  return (
    <button
      type="button"
      onClick={() => {
        router.push(`/sell/courses/manage/goals/${data && data.id}`);
      }}
      onMouseEnter={() => {
        if (titleRef.current) {
      ...
      <div className="flex h-full w-full flex-col justify-between p-4 text-left leading-normal">
        ...
        {data && data.status === 'draft' && (
          <div className="position-relative relative mt-2 h-6 w-full rounded-full">
            <div
              style={{ width: `${(data && data.progress / 10) * 100}%` }}
            ...
            <div className="top-50 absolute left-0 w-full text-center text-xs dark:text-white text-dark">
              {data && data.progress} / {10} completed
            </div>
          </div>
        )}
      </div>
