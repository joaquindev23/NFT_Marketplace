...
import SendTokensPolygon from '@/api/tokens/SendTokensPolygon';
import LoadingMoon from '@/components/loaders/LoadingMoon';
import Button from '@/components/Button';
import { resetSendTokens, sendTokensPolygon } from '@/redux/actions/crypto/crypto';

function classNames(...classes) {
  return classes.filter(Boolean).join(' ');
}

export default function SendPolygon({ wallet, txInfo }) {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      if (selected && selected.address.length === 42) {
        try {
          const res = await GetContractABIPolygon(selected.address);
      ...
      <div className="mt-4 rounded-lg border dark:border-dark-second border-gray-500 px-4 py-3 text-right dark:bg-dark-bg sm:px-6">
        ...
        {txInfo && txInfo.tx_hash ? (
          <div className="  py-5  ">
            ...
            <a
              href={`https://polygonscan.com/tx/${txInfo && txInfo.tx_hash}`}
              target="_blank"
              rel="noreferrer"
